...
class VAEVQ(nn.Module):
    ...
    def vq_forward(self, mean, std):
        z = rearrange(mean, 'b c h w -> b h w c').contiguous()
        ...
        epsilon = (z_q - mean) / std
        z_i = mean + std * epsilon.detach()
        z_quant = z_i + z_q - z_q.detach()
        return z_quant, epsilon, min_encoding_indices
