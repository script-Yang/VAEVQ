...
class VQLPIPSWithDiscriminator(nn.Module):
    ...
    def forward(self, codebook_loss, loss_break, inputs, c_rec, q_rec, optimizer_idx,
                global_step, last_layer=None, cond=None, split="train"):
        
        # now the GAN part
        if optimizer_idx == 0:
            c_rec_loss = torch.abs(inputs.contiguous() - c_rec.contiguous())
            q_rec_loss = torch.abs(inputs.contiguous() - q_rec.contiguous())
            rec_loss = (c_rec_loss + q_rec_loss)/2.0
            nll_loss = rec_loss.clone()
            if self.perceptual_weight > 0:
                # p_loss = self.perceptual_loss(inputs.contiguous(), reconstructions.contiguous())
                c_p_loss = self.perceptual_loss(inputs.contiguous(), c_rec.contiguous())
                q_p_loss = self.perceptual_loss(inputs.contiguous(), q_rec.contiguous())
                p_loss = (c_p_loss + q_p_loss)/2.0
            ...
            if cond is None:
                ...
                c_logits_fake = self.discriminator(c_rec.contiguous())
                q_logits_fake = self.discriminator(q_rec.contiguous())
                logits_fake = (c_logits_fake + q_logits_fake)/2.0
            else:
                assert self.disc_conditional
                # logits_fake = self.discriminator(torch.cat((reconstructions.contiguous(), cond), dim=1))
                c_logits_fake = self.discriminator(torch.cat((c_rec.contiguous(), cond), dim=1))
                q_logits_fake = self.discriminator(torch.cat((q_rec.contiguous(), cond), dim=1))
                logits_fake = (c_logits_fake + q_logits_fake)/2.0
            ...
            ### Define 2025.11.20
            scale_codebook_loss = self.commit_weight * codebook_loss #entropy_loss
            loss = nll_loss + g_loss + scale_codebook_loss
            if disc_factor == 0:
        ...
        if optimizer_idx == 1:
            ...
            if cond is None:
                ...
                c_logits_fake = self.discriminator(c_rec.contiguous().detach())
                q_logits_fake = self.discriminator(q_rec.contiguous().detach())
                logits_fake = (c_logits_fake + q_logits_fake)/2.0
            else:
                logits_real = self.discriminator(torch.cat((inputs.contiguous().detach(), cond), dim=1))
                # logits_fake = self.discriminator(torch.cat((reconstructions.contiguous().detach(), cond), dim=1))
                c_logits_fake = self.discriminator(torch.cat((c_rec.contiguous().detach(), cond), dim=1))
                q_logits_fake = self.discriminator(torch.cat((q_rec.contiguous().detach(), cond), dim=1))
                logits_fake = (c_logits_fake + q_logits_fake)/2.0
