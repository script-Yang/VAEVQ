...
def get_args():
    ...
    parser.add_argument("--config_file", default='/data01/home/ysc/SimVQ/configs/imagenet_simvq_128_B.yaml', type=str)
...
def main(args):
    ...
    recons_save_dir = Path(args.config_file).parent /  "recons_uitl_low"
    ...
    #mypath = '/data01/home/ysc/flower/valid'

    val_loader = load_data(dataset_path=mypath,
    image_size=256,batch_size=1,state=False,
        ...
        for i,batch in tqdm(enumerate(val_loader),total=len(val_loader)):
            ...
            if model.use_ema:
                with model.ema_scope():
                    ...
                    z_q, z_c, indices, loss_break = model.encode(images)
                    reconstructed_images = model.decode(z_q)
            else:
            #    quant, diff, indices, _ = model.encode(images)
            #    reconstructed_images = model.decode(quant)
                z_q, z_q, indices, loss_break = model.encode(images)
                reconstructed_images = model.decode(z_q)
